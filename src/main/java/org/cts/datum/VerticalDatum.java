 ...
 * There are three kinds of vertical datum :
 * <ul>
 *     <li>tidal, based on sea levels</li>
 *     <li>gravimetric, based on the geoid</li>
 *     <li>geodetic, based on the ellipsoid associated to a geodetic datum</li>
 * </ul>
 *
...
public class VerticalDatum extends AbstractDatum {
    ...
    //-------------------------------------------------------------------------
    // List of Ellipsoidal Vertical Datum based on known ellipsoid
    //-------------------------------------------------------------------------
    ...
    //-------------------------------------------------------------------------
    // List of vertical datum based Spherical Harmonic Models
    //-------------------------------------------------------------------------

    /**
     * EGM2008 model described as a grid on the France territory only
     */
    public final static VerticalDatum EGM2008 = new VerticalDatum(
            new Identifier("EPSG", "????", "EGM2008", "EGM2008"),
            new GeographicExtent("France", 42, 51.5, -5.5, 8.5),
            "",
            "2008", Type.GEOIDAL, "egm08fr.mnt", GeodeticDatum.WGS84);

    //-------------------------------------------------------------------------
    // List of geoidal vertical datum including a transformation definition
    // to/from a ellipsoidal datum
    //-------------------------------------------------------------------------
    /**
    ...
    //@TODO Following altimetric systems do not use RGF93 which is only valid in metropolitan France
    // WGS84 would be more appropriate, but GeodeticDatum should not be part of the VerticalDatum !
    ...
    /**
     * Returns the type of this vertical datum. Default is "geoidal".
     */
    public Type getType() {
        return type;
    }
     ...
     * ellipsoidal height.
     */
    //@TODO not consistent with GeodeticDatum management.
    // Create a get/set ToEllipoidHeight method returning a transformation
    // to ellipoidal heights in WGS84 or equivalent (ETRS89; ITRF, RGF93...)
    // Not sure it is necessaru to manage maps from any VD to any VD as we do
     ...
     */
    // @TODO : we should replace this by getGeodeticDatum
    // - for ellipsoidal height -> geodetic datum defining the ellipsoid used as a reference
    // - for tidal altitudes -> geodetic datum used by the get/set ToEllipsoidHeight
     ...
     */
    // @TODO : I think that toWGS84 makes no sense here (see above)
     ...
     */
    // @TODO : a vertical datum definition should not depend on a Prime Meridian
